...
$Global:DrModulePath = Join-Path $Global:DrBin 'DrModuleV3.psm1'
$Global:DrLogo = Join-Path $Global:DrBin 'Logo.jpg'
$Global:DrVarStorePath = Join-Path $Global:DrBin 'variables.xml'
$Global:DrFilePusherPath = 'C:\ProgramData\Syncro\bin\FilePusher.exe'  # Fixed location
$Global:DrRepairTechKabutoApiUrl = 'https://rmm.syncromsp.com'

# Hidden Root (Operational Workspace)
...
# Jobs Path (Dynamic during runtime)
$Global:DrJobRoot = $Global:DrJobsPath   # Will be set by Initialize-Environment
$Global:DrLogFile = $null    # Will be set by Initialize-Job or Initialize-Environment
$Global:DrTimers = $null    # Will be set by Initialize-Job or Initialize-Environment

if (-not $Global:DrComputer) { 
    #$Global:DrComputer = Get-ComputerInfo
}
...
function Update-JobPathsAfterTicket {
    ...
    try {
        ...
        $logParams = Get-LogEntryParams -Buffer -FlushBuffer:$FlushBuffer
        $detailsParams = $logParams.Details
        $summaryParams = $logParams.Summary
        ...
        if (Test-Path $oldJobRoot) {
            ...
            Add-LogEntry -Message "Job root folder not found: $oldJobRoot" -Icon 'Warning' @summaryParams
            #return
        }
        ...
        if ($oldLogFile) {
            ...
            #Add-LogEntry -Message "Updated log file path to: $Global:DrLogFile" -Icon 'FileHandling' @detailsParams
        }

        Add-LogEntry -Message "✅ Updated global paths: DrJobRoot=$Global:DrJobRoot, DrLogs=$Global:DrLogs, DrTemp=$Global:DrTemp" -Icon 'FileHandling' @summaryParams
    }
    catch {
...
#endregion Stable Functions

#region In Progress Functions
#region Logging Core
...
class DrRecommendation {
    ...
    [int] GetNextNumericId([object[]] $existing) {
    ...
    [bool] Approve([DrRecommendation[]] $allRecommendations) {
    ...
    [bool] IsDuplicateCommand([DrRecommendation[]] $allRecommendations) {
        $existing = $allRecommendations | Where-Object {
            $_.CommandToRun -eq $this.CommandToRun -and $_.Approved -eq $true -and $_.NumericId -ne $this.NumericId
        }
...
# Path should already be set in your module/session
# $Global:DrRecommendations = "C:\ProgramData\DrModuleV3\DrRecommendations.json"
...
#endregion DrRecommendation
#region Timer Management Functions
# Version: 3.4
# Description: Timer management functions with lifecycle, ticket integration, and structured logging.
# Change: StartTime stored as ISO 8601 string for simplicity and reliability.
# Enhancement: All lifecycle functions support *ALL for Name parameter.
...
#endregion In Progress Functions

#endregion DrModuleV3 - Stable Functions
...
function Write-ObjProperties {
    ...
    if ($Level -eq 0) {
        ...
        Add-LogEntry -Message "${indent}$($ObjCount.Value) object properties listed." -Icon 'task' @summaryParams
    }
}
...
function Get-EventLogsByLevelAndTime {
    ...
    if (-not $LogName -and -not $AllLogs) {
        ...
        Add-LogEntry -Message "Defaulting to primary logs: System, Application, Security" -Icon 'info' @detailsParams 
    }
...
function Reset-WindowsUpdate {
    ...
    try {
        ...
        Add-LogEntry "Stopping services..." @detailsParams
        Get-Service -Name wuauserv, cryptSvc, bits, msiserver | Stop-Service -Force -ErrorAction Stop
    }
    catch {
        Add-LogEntry "Error stopping services: $_" @summaryParams
        throw
    }

    try {
        # Clear the BITS queue
        Add-LogEntry "Clearing BITS queue..." @detailsParams
        Remove-Item -Path "$env:ALLUSERSPROFILE\Application Data\Microsoft\Network\Downloader\*.*" -Force -ErrorAction Stop
    }
    catch {
        Add-LogEntry "Error clearing BITS queue: $_" @summaryParams
        throw
    }

    try {
        # Remove directories
        Add-LogEntry "Removing directories..." @detailsParams
        ...
        Add-LogEntry "Error removing directories: $_" @summaryParams
        throw
    }

    try {
        # Start services
        Add-LogEntry "Starting services..." @detailsParams
        Get-Service -Name wuauserv, cryptSvc, bits, msiserver | Start-Service -ErrorAction Stop
    }
    catch {
        Add-LogEntry "Error starting services: $_" @summaryParams
        throw
    }

    try {
        # Check for updates
        Add-LogEntry "Checking for updates..." @detailsParams
        Get-WindowsUpdates -ErrorAction Stop
    }
    catch {
        Add-LogEntry "Error checking for updates: $_" @summaryParams
        throw
    }
...
# Return rich info from Install-UpdateWinget so you don’t need a second Get-Command
# - Preserves your single summary line + AddToBody/AddBody behavior
# - No Mandatory parameters; PowerShell 5-safe
# - Uses try/catch everywhere
# - Returns a PSCustomObject with fields you can consume
